...
### 私有空间（private.html，仅自己可见）

博客里不便公开的笔记可以放进**独立私有仓库**（如 `ylzh-private`），只有你的浏览器（有令牌）能访问：

- **私有页**：`private.html`——没有令牌时显示 404；有令牌时从私有仓库拉取笔记列表，支持搜索、阅读（与公开文章相同的渲染管线：Markdown + KaTeX + 定理 + 宏）
- **为什么用独立私有仓库**：公开仓库（GitHub Pages 免费版必须公开）里的所有文件、分支、历史都公开；私有内容必须放在独立的私有仓库里
- **怎么添加私有笔记**：`git clone git@github.com:ylzhang93/ylzh-private.git` → 在 `posts/` 下加 `.md`（结构同公开站：`posts/index.json` 清单 + `posts/*.md`）→ push；或用 GitHub 网页编辑器直接编辑
- **令牌权限**：令牌需要私有仓库的读取权限（fine-grained 令牌请勾选 `ylzh-private` 的 `Contents: Read`）
- 私密内容**绝不写入公开仓库**——这就是"物理级"保密

### 草稿 / 私有 / 公开（双仓库模型）

写作页有**状态选择**：草稿、私有、公开。内容按状态存放在不同仓库：

| 状态 | 存放位置 | 访客能看到吗 |
| --- | --- | --- |
| 草稿 | 私有仓库 `ylzh-private` | ❌ 列表/搜索/私有空间都不显示，只有写作页能编辑 |
| 私有 | 私有仓库 `ylzh-private` | ❌ 公开站看不到；私有空间 `private.html`（有令牌）可看 |
| 公开 | 公开仓库 `ylzh` | ✅ 博客列表、搜索、文章页 |

**保存时文件自动在两个仓库间移动**：把草稿/私有改成"公开"再保存 → 文件移入 `ylzh` 并从私有仓库删除；把公开改成"私有/草稿"再保存 → 反向移动。两个仓库的清单自动同步。

- 新文章**默认草稿**（安全默认，写完再决定发布）
- 写作页「编辑已有文章」下拉框会合并两个仓库的文章，带 公开/私有/草稿 标记
- 私有空间 `private.html` 只显示"私有"状态；草稿不在那里显示
- 博客页（站长视角）的「还有 N 篇草稿」提示从私有仓库读取
- 命令行：`python new_post.py "标题" --draft`（草稿写入本地，推送前记得同步）
