...
- 写作页勾选 **「保存为草稿（访客不可见）」** → 文章存为草稿：**博客列表不显示、搜索搜不到、文章页显示"尚未发布"**，访客完全看不到
- 草稿在写作页「编辑已有文章」下拉框里带 **「草稿」** 标记，选择后可继续编辑；取消勾选再保存即发布
- 博客页（站长视角）会显示「还有 N 篇草稿」提示
- 命令行：`python new_post.py "标题" --draft`
